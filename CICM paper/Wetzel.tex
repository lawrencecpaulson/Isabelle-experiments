...
The next three definitions constrain the next the value of $\epsilon$. To ensure convergence, each approximant needs to be close to the previous one, hence a neighbourhood (open ball) whose radius has a factorial in the denominator. \isa{DD} is the intersection of this neighbourhood with the set~$D$, excluding the one point that must be avoided. Finally, the function \isa{dd} picks an arbitrary element of~\isa{DD}\@.
...
Because $D$ is dense in $\mathbb{C}$, it's possible to show that \isa{DD} is always nonempty and therefore that \isa{dd} always picks out an element satisfying the constraints described above.
...
It's bothersome that about 20 lines of pure boilerplate are needed to code up the formal application of transfinite recursion to the construction formalised above. Skipping over this, let's look at the conclusion of this entire proof.
...
\end{align*}
and that both parts of the union are finite: the first is a collection below $\alpha$, a countable ordinal, and the second is a subset of $D$, a countable set.
...
Both of these involve Kevin Buzzard and his colleagues formalising the highly sophisticated work of Fields Medallist Peter Scholze, using Lean.

The small example reported here (350 lines; about three weeks' work) cannot be compared with such accomplishments, but shines a light on one issue that makes mathematics difficult: its interconnectedness. If your problem concerns itself with sets of permutations, you might suddenly find yourself needing group theory. If you are trying to count the elements of the group, you might find yourself requiring sophisticated combinatorial arguments. If you find yourself talking about countable sets (and countability is an everyday notion), you could find yourself wandering into the world of cardinals, then into the world of ordinals and then perhaps into the undecidable.
...
The intriguing solution to Wagner's problem by Erd\H{o}s was easily formalised in Isabelle/HOL, using a library designed to integrate set theory with higher-order logic.
The ease with which we can intermix analytic functions with ordinals gives reason to hope that more advanced problems will be amendable to formalisation without particular difficulties.
