...
(*The "sufficiently large" assumptions are problematical.
  The calculation for \<gamma> > 3/20 is sharp. We need a finite gap for the limit to exist. 
  We can get away with 1/300. *)
...
lemma error_10_2_True: "\<forall>\<^sup>\<infinity>k. ok_fun_95b k + (3/20 + 1/300) * real k / real 30 \<ge> k/200"
...
definition "Big_Far_10_2 \<equiv> \<lambda>\<mu> l. Big_Far_9_3 \<mu> l \<and> Big_Far_9_5 \<mu> l
                \<and> (\<forall>k\<ge>l. ok_fun_95b k + (if \<mu> > (3/20 + 1/300) then \<mu>*k/30 else \<mu>*k/15) \<ge> k/200)"
...
proof -
  have T: "\<forall>\<^sup>\<infinity>l. \<forall>k\<ge>l. (\<forall>\<mu>. (3/20 + 1/300) \<le> \<mu> \<and> \<mu> \<le> \<mu>1 \<longrightarrow> k/200 \<le> ok_fun_95b k + \<mu>*k / real 30)"
    using assms 
  ...
  have "\<forall>\<^sup>\<infinity>l. \<forall>k\<ge>l. (\<forall>\<mu>. 1/10 \<le> \<mu> \<and> \<mu> \<le> \<mu>1 \<longrightarrow> k/200 \<le> ok_fun_95b k + (if \<mu> > (3/20 + 1/300) then \<mu>*k/30 else \<mu>*k/15))"
    using assms
...
proof -
  ...
  have ok95: "ok_fun_95b k + \<gamma> * real k / 30 \<ge> k/200"
  ...
  proof (cases "\<gamma> > 3/20 + 1/300")
    case True
    then have "\<delta> * k - ok_fun_95b k \<le> (\<gamma>/30) * k"
      using \<open>k>0\<close> ok95 by (simp add: \<delta>_def)
  ...
  next
    case False
    have "\<delta> * k - ok_fun_95b k \<le> (\<gamma>/30) * k"
      using \<open>k>0\<close> ok95 by (simp add: \<delta>_def)
    also have "\<dots> \<le> 3 * \<gamma> * (real t)\<^sup>2 / (40*k)"
    ...
    also have "\<dots> \<le> 3 * \<gamma> * (real t)\<^sup>2 / (20*k)"
      using \<gamma> by (simp add: divide_simps)
    finally have \<dagger>: "\<delta>*k - ok_fun_95b k \<le> 3 * \<gamma> * (real t)\<^sup>2 / (20*k)" .

    have "gamf \<gamma> \<ge> gamf (3/20 + 1/300)"
      apply (intro DERIV_nonpos_imp_nonincreasing[of \<gamma> "3/20 + 1/300" gamf] )
      using False apply (force simp add: )
      apply (rule deriv_gamf [of \<gamma>])
      using False \<gamma>
         apply (auto simp: )
      done
    moreover have "ln (gamf (3/20 + 1/300)) \<ge> -1/3 + 1/10"
      unfolding gamf_def by (approximation 6)
    moreover have "gamf (3/20 + 1/300) > 0"
      by (simp add: gamf_def)
