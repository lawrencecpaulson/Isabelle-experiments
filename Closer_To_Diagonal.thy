...
(*The body of the proof is there in full, but the "sufficiently large" assumptions are problematical.
The calculation for \<gamma> > 3/20 is sharp. We seem to need a finite gap for the limit to exist. *)
...
definition "Big_Far_10_2 \<equiv> \<lambda>\<mu> l. Big_Far_9_3 \<mu> l \<and> Big_Far_9_5 \<mu> l
                \<and> (\<forall>k\<ge>l. ok_fun_95b k + (if \<mu> > 3/20 then \<mu>*k/30 else \<mu>*k/15) \<ge> k/200)"
...
proof -
  have "\<forall>\<^sup>\<infinity>l. \<forall>k\<ge>l. (\<forall>\<mu>. 1/10 \<le> \<mu> \<and> \<mu> \<le> \<mu>1 \<longrightarrow> k/200 \<le> ok_fun_95b k + (if \<mu> > 3/20 then \<mu>*k/30 else \<mu>*k/15))"
    using assms 
    apply (split if_split)
    unfolding eventually_conj_iff all_imp_conj_distrib all_conj_distrib imp_conjL [symmetric]
    apply (intro conjI eventually_all_ge_at_top eventually_all_geI0 )
     apply (rule eventually_mono [OF error_10_2])
