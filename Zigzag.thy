...
lemma (in Book) ZZ_8_1:
  assumes \<mu>: "0<\<mu>" "\<mu><1" and big: "Big_ZZ_8_1 \<mu> l" 
  defines "\<S>\<S> \<equiv> dboost_star \<mu>" and "\<R> \<equiv> Step_class \<mu> {red_step}"
  defines "sum_\<S>\<S> \<equiv> (\<Sum>i\<in>\<S>\<S>. (1 - beta \<mu> i) / beta \<mu> i)"
...
proof -
  ...
  have "(\<Sum>h=Suc 0..maxh. \<Sum>i<m. \<Delta>\<Delta> i h / alpha h) \<le> (\<Sum>h=Suc 0..maxh. 1)"
  proof (intro sum_mono)
    fix h
    assume "h \<in> {Suc 0..maxh}"
    have "(\<Sum>i<m. \<Delta>\<Delta> i h) \<le> alpha h"
    ...
    then show "(\<Sum>i<m. \<Delta>\<Delta> i h / alpha h) \<le> 1"
  ...
  finally have 34: "(\<Sum>h=Suc 0..maxh. \<Sum>i<m. \<Delta>\<Delta> i h / alpha h) \<le> 1 + 2 * ln k / eps k" .
  ...
  have "oneminus * sum_\<S>\<S>
     = (\<Sum>i\<in>\<S>\<S>. oneminus * ((1 - beta \<mu> i) / beta \<mu> i))"
    using sum_distrib_left sum_\<S>\<S>_def by blast
  also have "\<dots> \<le> (\<Sum>i\<in>\<S>\<S>. \<Sum>h=1..maxh. \<Delta>\<Delta> i h / alpha h)"
    by (intro sum_mono 35)
  also have "\<dots> = (\<Sum>h=1..maxh. \<Sum>i\<in>\<S>\<S>. \<Delta>\<Delta> i h / alpha h)"
  ...
  have m_eq: "{..<m} = \<R> \<union> \<S> \<union> (\<B> \<union> \<D>)"
    using before_halted_eq \<open>\<mu>>0\<close>
...
lemma (in Book) ZZ_8_5:
  assumes \<mu>: "0<\<mu>" "\<mu><1" and big: "Big_ZZ_8_5 \<mu> l" 
  defines "\<R> \<equiv> Step_class \<mu> {red_step}" and "\<S> \<equiv> Step_class \<mu> {dboost_step}"
  shows "card \<S> \<le> (bigbeta \<mu> / (1 - bigbeta \<mu>)) * card \<R> 
        + (2 / (1-\<mu>)) * k powr (19/20)"
proof -
  ...
  also have "\<dots> \<le> 3 * eps k powr (1/4) * k"
    using \<mu> big X_7_5 by (auto simp: Big_ZZ_8_5_def \<S>\<S>_def \<S>_def)
  ...
  have bigbeta_lt1: "bigbeta \<mu> < 1"
    and bigbeta_gt0: "0 < bigbeta \<mu>"
    and beta_gt0: "\<And>i. i \<in> \<S> \<Longrightarrow> beta \<mu> i > 0" 
    using bigbeta_ge0 big \<mu> 
    by (auto simp: Big_ZZ_8_5_def \<S>_def beta_gt0 bigbeta_gt0 bigbeta_less1)
  then have ge0: "bigbeta \<mu> / (1 - bigbeta \<mu>) \<ge> 0"
    by auto
  show ?thesis
  proof (cases "\<S>\<S> = {}")
    case True
    ...
    also have "\<dots> \<le> (2 / (1-\<mu>)) * k powr (19/20)"
      using \<mu> kn0 by (simp add: divide_simps)
    finally show ?thesis
      by (smt (verit, ccfv_SIG) mult_nonneg_nonneg of_nat_0_le_iff ge0) 
  next
    case False    
    have bb_le: "bigbeta \<mu> \<le> \<mu>"
      using big bigbeta_le [OF \<mu>] by (auto simp: Big_ZZ_8_5_def)
    have "(card \<S> - k powr (19/20)) / bigbeta \<mu> \<le> card \<S>\<S> / bigbeta \<mu>"
      by (smt (verit) "*" \<mu> bigbeta_ge0 divide_right_mono)
    also have "\<dots> = (\<Sum>i\<in>\<S>\<S>. 1 / beta \<mu> i)"
    ...
    proof -
      have "(\<Sum>i\<in>\<S>\<S>. (1 - beta \<mu> i) / beta \<mu> i)
            \<le> real (card \<R>) + k powr (19/20)"
        using ZZ_8_1 big \<mu> 
        unfolding Big_ZZ_8_5_def \<R>_def \<S>\<S>_def by blast
      moreover have "(\<Sum>i\<in>\<S>\<S>. beta \<mu> i / beta \<mu> i) = (\<Sum>i\<in>\<S>\<S>. 1)"
    ...
    finally have "(card \<S> - k powr (19/20)) / bigbeta \<mu> \<le> real (card \<S>) + card \<R> + k powr (19/20)" .
    then have "card \<S> - k powr (19/20) \<le> (real (card \<S>) + card \<R> + k powr (19/20)) * bigbeta \<mu>"
      using bigbeta_gt0 by (simp add: field_simps)
    then have "card \<S> * (1 - bigbeta \<mu>) \<le> bigbeta \<mu> * card \<R> + (1 + bigbeta \<mu>) * k powr (19/20)"
      by (simp add: algebra_simps)
    then have "card \<S> \<le> (bigbeta \<mu> * card \<R> + (1 + bigbeta \<mu>) * k powr (19/20)) / (1 - bigbeta \<mu>)"
      using bigbeta_lt1 by (simp add: field_simps)
    also have "\<dots> = (bigbeta \<mu> / (1 - bigbeta \<mu>)) * card \<R> 
                  + ((1 + bigbeta \<mu>) / (1 - bigbeta \<mu>)) * k powr (19/20)"
      using bigbeta_gt0 bigbeta_lt1 by (simp add: divide_simps)
    also have "\<dots> \<le> (bigbeta \<mu> / (1 - bigbeta \<mu>)) * card \<R> + (2 / (1-\<mu>)) * k powr (19/20)"
...
lemma (in Book) ZZ_8_6:
  assumes \<mu>: "0<\<mu>" "\<mu><1" and big: "Big_ZZ_8_6 \<mu> l" 
  defines "\<R> \<equiv> Step_class \<mu> {red_step}" and "\<S> \<equiv> Step_class \<mu> {dboost_step}"
    and "a \<equiv> 2 / (1-\<mu>)"
  assumes s_ge: "card \<S> \<ge> k powr (39/40)"
  shows "bigbeta \<mu> \<ge> (1 - a * k powr (-1/40)) * (card \<S> / (card \<S> + card \<R>))"
proof -
  have bigbeta_lt1: "bigbeta \<mu> < 1" and bigbeta_gt0: "0 < bigbeta \<mu>"
    using bigbeta_ge0 big \<mu> 
  ...
  have s_gt_a: "a * k powr (19/20) < card \<S>"
        and 85: "card \<S> \<le> (bigbeta \<mu> / (1 - bigbeta \<mu>)) * card \<R> + a * k powr (19/20)"
  ...
  then have "(card \<S> - a * k powr (19/20)) / card \<R> \<le> bigbeta \<mu> / (1 - bigbeta \<mu>)"
    using 85 bigbeta_gt0 bigbeta_lt1 t_non0 by (simp add: pos_divide_le_eq)
  then have "bigbeta \<mu> \<ge> (1 - bigbeta \<mu>) * (card \<S> - a * k powr (19/20)) / card \<R>"
    by (smt (verit, ccfv_threshold) bigbeta_lt1 mult.commute le_divide_eq times_divide_eq_left)
  then have *: "bigbeta \<mu> * (card \<R> + card \<S> - a * k powr (19/20)) \<ge> card \<S> - a * k powr (19/20)"
