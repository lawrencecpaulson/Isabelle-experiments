...
locale gmonoid =
  fixes M :: "'i::monoid_add \<Rightarrow> 'a set" and composition (infixl "\<cdot>" 70) and unit ("\<one>")
  assumes composition_closed [intro, simp]: "\<lbrakk> a \<in> M i; b \<in> M j\<rbrakk> \<Longrightarrow> a \<cdot> b \<in> M(i+j)"
    and unit_closed [intro, simp]: "\<one> \<in> M 0"
...
locale ggroup =
  gmonoid G "(\<cdot>)" \<one> for G and composition (infixl "\<cdot>" 70) and unit ("\<one>") +
  assumes invertible [simp, intro]: "u \<in> G i \<Longrightarrow> monoid.invertible (G i) (\<cdot>) \<one> u"
...
locale gring = multiplicative: gmonoid R "(\<cdot>)" \<one>
...
lemma zero: "\<zero> \<in> R i"
  by (metis abelian_group_def ag commutative_gmonoid_def commutative_monoid_def gmonoid.unit_closed)

lemma "\<lbrakk>a \<in> R i; b \<in> R i\<rbrakk> \<Longrightarrow> a \<oplus> b \<in> R i"
  by (metis Group_Theory.group_def abelian_group_def ag ggroup_def gmonoid.composition_closed)

lemma "\<lbrakk>a \<in> R i; b \<in> R j\<rbrakk> \<Longrightarrow> a \<cdot> b \<in> R (i+j)"
...
text \<open>p 29, ll 27--28\<close>
lemma sub [intro, simp]:
  "a \<in> N i \<Longrightarrow> a \<in> M i"
  using subset by blast

text \<open>p 29, ll 32--33\<close>
sublocale sub: gmonoid N "(\<cdot>)" \<one>
  by unfold_locales (auto simp: sub_composition_closed sub_unit_closed)
...
theorem submonoid_transitive:
  assumes "gsubmonoid K N composition unit"
    and "gsubmonoid N M composition unit"
  shows "gsubmonoid K M composition unit"
proof -
  interpret K: gsubmonoid K N composition unit by fact
  interpret M: gsubmonoid N M composition unit by fact
...
lemma submonoid_invertible [intro, simp]:
  "\<lbrakk> sub.invertible u; u \<in> N \<rbrakk> \<Longrightarrow> sup.invertible u"
  using sup.invertibleI by blast

text \<open>p 31, l 7\<close>
lemma submonoid_inverse_closed [intro, simp]:
  "\<lbrakk> sub.invertible u; u \<in> N \<rbrakk> \<Longrightarrow> sup.inverse u \<in> N"
  using sup.inverse_equality by auto
